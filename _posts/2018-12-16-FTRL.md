...
*   <span class="s1">사용자가 **검색 q**를 할 때, **초기 후보 집합**은 </span>광고는 광고주가 **선택한 키워드**를 기반으로 하는 **쿼리 q**와 일치합니다.   
    ![](./ftrl_files/image2018-11-6_19-49-56.png "인텔리전스랩스 > 20181107_이상열_FTRL Proximal > image2018-11-6_19-49-56.png")![](./ftrl_files/image2018-11-6_19-50-16.png "인텔리전스랩스 > 20181107_이상열_FTRL Proximal > image2018-11-6_19-50-16.png")  
...
![](./ftrl_files/스크린샷 2018-11-04 오후 3.05.59.png "인텔리전스랩스 > 20181107_이상열_FTRL Proximal > 스크린샷 2018-11-04 오후 3.05.59.png")
...
*   그러나 훈련 방법은 Google Brain 팀에서 설명한 **Downpour SGD** 방법과 유사하지만 많은 계층의 네트워크가 아닌 **단일 계층 모델**을 훈련 한다는 차이점이 있습니다. 
    ...
    <table class="wysiwyg-macro" data-macro-name="info" data-macro-id="749b17ad-07f1-41d0-961f-2fc6f76401eb" data-macro-parameters="title=요약" data-macro-schema-version="1" style="background-image: url(/plugins/servlet/confluence/placeholder/macro-heading?definition=e2luZm86dGl0bGU97JqU7JW9fQ&amp;locale=ko_KR&amp;version=2); background-repeat: no-repeat;" data-macro-body-type="RICH_TEXT">
...
*   <span class="s1">로지스틱 회귀를 사용하여 문제를 모델링 하고자 한다면 다음 온라인 프레임 워크를 따릅니다.</span>
    ...
    *   <span class="s1">pt = σ (wt · xt)를 예측한다. 여기서 σ (a) = 1 / (1 + exp (-a))는 시그 모이드 함수이다. 그런 다음 레이블 yt ∈ {0, 1}을 관찰하고 결과 LogLoss </span>
        ...
        ![](./ftrl_files/스크린샷 2018-11-04 오후 3.45.59.png "인텔리전스랩스 > 20181107_이상열_FTRL Proximal > 스크린샷 2018-11-04 오후 3.45.59.png")
...
*   <span class="s1">Stochastic gradient descent는 이러한 종류의 문제에 대해 매우 효과적이며, **최소한의 컴퓨팅 리소스**로 뛰어난 **예측 정확도**를 제공합니다. </span>
    ...
    **<span>SGD (Stochastic Gradeint Descent): </span>**<span>SGD는 Gradient Descent가 갖는 너무 큰 계산량 문제를 해결하기 위해 전체 데이터중에서 랜덤하게 추출한 1/N의 데이터만을 사용해 훨씬 더 빠르게, 자주 업데이트하는 방법이다.</span>
...
*   <span class="s1">손실의 그라디언트에 **L1 페널티의 하위 그라디언트**를 추가하는 것만으로도 일부 해결 되었습니다. <span>Forward-Backward Splitting(FOBOS)과 같이</span> 절단된 그래디언트와 같은 보다 정교한 접근법은 희소성을 도입하는 데 성공합니다.</span>
    ...
    <span class="s1">![](./ftrl_files/스크린샷 2018-11-04 오후 3.55.13.png "인텔리전스랩스 > 20181107_이상열_FTRL Proximal > 스크린샷 2018-11-04 오후 3.55.13.png")  
    </span>
...
![](./ftrl_files/스크린샷 2018-11-04 오후 4.04.39.png "인텔리전스랩스 > 20181107_이상열_FTRL Proximal > 스크린샷 2018-11-04 오후 4.04.39.png")
...
*   FTRL Proximal == Online Gradient Descent(OGD) + Regularized Dual Averaging(RDA)
    ...
    *   **첫 번째 term g1:t 는 the gradient part, 이전 샘플 1~t에 대한 그래디언트 합계.** (해당 샘플에서 가설의 손실을 줄이면 그 방향으로 이동)
        *   ![](./ftrl_files/image2018-11-4_16-11-27.png "인텔리전스랩스 > 20181107_이상열_FTRL Proximal > image2018-11-4_16-11-27.png")
    ...
    *   **세 번째 term은 정규화 부분 (기존의 l1 norm, <span style="color: rgb(81,81,81);">lasso)</span>**  
...
*   <span class="s1">learning rate 스케줄의 관점에서 σs를 정의한다</span>
    ...
    *   <span class="s1">learning rate : ![](./ftrl_files/image2018-11-4_16-11-52.png "인텔리전스랩스 > 20181107_이상열_FTRL Proximal > image2018-11-4_16-11-52.png")![](./ftrl_files/image2018-11-4_16-12-0.png "인텔리전스랩스 > 20181107_이상열_FTRL Proximal > image2018-11-4_16-12-0.png").</span>
...
*   <span class="s1">빠른 검사에서 **FTRL-Proximal 업데이트**가 그라디언트 디센트보다 구현하기가 어렵거나 모든 과거 계수를 저장해야 한다고 생각할 수 있습니다. </span>
    ...
    *   <span class="s1">그러나 실제로 계수 당 하나의 숫자만 저장하면 됩니다. ( re-write the update as the argmin over w ∈ R d )</span>
        ...
        <span class="s1">![](./ftrl_files/image2018-11-4_16-13-49.png "인텔리전스랩스 > 20181107_이상열_FTRL Proximal > image2018-11-4_16-13-49.png")  
        </span>

        [![](./ftrl_files/스크린샷 2018-11-04 오후 4.18.22.png "인텔리전스랩스 > 20181107_이상열_FTRL Proximal > 스크린샷 2018-11-04 오후 4.18.22.png")](http://jhymyr.tistory.com/entry/FTRL)

        ![](./ftrl_files/image2018-11-4_16-14-43.png "인텔리전스랩스 > 20181107_이상열_FTRL Proximal > image2018-11-4_16-14-43.png")
...
*   FTRL-Proximal 함수를 w에 대해서 최소화 하기 위해서 미분하면 wt + 1, i에 대해 per coordinate base에서 closed 형태로 풀면
    ...
    *   ### Per-coordinate learning rate 
        ...
        *   #### [Per-parameter adaptive learning rates  (**Adagrad, **RMSprop, Adam)****](http://aikorea.org/cs231n/neural-networks-3/#sgd)
        ...
        <div>![](./ftrl_files/FTRL_algo.png "인텔리전스랩스 > 20181107_이상열_FTRL Proximal > FTRL_algo.png")</div>

## **Experimental Results. **

![](./ftrl_files/스크린샷 2018-11-04 오후 4.49.07.png "인텔리전스랩스 > 20181107_이상열_FTRL Proximal > 스크린샷 2018-11-04 오후 4.49.07.png")

*   <span class="s1">간단한 휴리스틱에 대해 FTRL-Proximal을 테스트하기 위해 매우 큰 데이터 세트를 실행했습니다. </span>
...
*   <span class="s1">온라인 그래디언트 디센트에 대한 **표준 이론은 global하게 learning rate의  스케줄 ![](./ftrl_files/image2018-11-4_16-12-0.png "인텔리전스랩스 > 20181107_이상열_FTRL Proximal > image2018-11-4_16-12-0.png")를 사용하는 것을 제안합니다, 이것은 모든 좌표에 공통적인 것**</span>
...
*   로지스틱 회귀를 사용하여 10 개의 동전에 대해 **Pr(heads | coin i)**을 추정한다고 가정 해보십시오.
    ...
    *   여기서, 문제 i에 대한 알고리즘 인스턴스는 ![](./ftrl_files/image2018-11-4_16-12-0.png "인텔리전스랩스 > 20181107_이상열_FTRL Proximal > image2018-11-4_16-12-0.png") 같은 학습 률을 사용합니다. 
...
![](./ftrl_files/image2018-11-4_17-11-10.png "인텔리전스랩스 > 20181107_이상열_FTRL Proximal > image2018-11-4_17-11-10.png")
...
*   <span class="s1">q2.13 인코딩에서 우리는 두 비트를</span>**2 진 소수점, 2 진 소수점 오른쪽에 13 비트,** 부호 당 하나의 비트 **(값 당 총 16 비트 사용)**가 있습니다.
    ...
    *   ![](./ftrl_files/스크린샷 2018-11-04 오후 5.21.19.png "인텔리전스랩스 > 20181107_이상열_FTRL Proximal > 스크린샷 2018-11-04 오후 5.21.19.png")
...
*   로지스틱 회귀 분석을 사용하면, P(positive)의 이벤트에 대한 기울기는 p - 1이고 음(negative)의 이벤트에 대한 기울기는 p이며  
    ![](./ftrl_files/image2018-11-4_17-35-28.png "인텔리전스랩스 > 20181107_이상열_FTRL Proximal > image2018-11-4_17-35-28.png")
...
*   <span style="color: black;">A fraction r ∈ (0, 1] of the queries where none of the ads were clicked. </span>  
    ...
    *   ![](./ftrl_files/image2018-11-4_17-46-59.png "인텔리전스랩스 > 20181107_이상열_FTRL Proximal > image2018-11-4_17-46-59.png")
...
*   샘플링되지 않은 데이터에서 **무작위로 선택된 이벤트 t**가 서브 샘플링 된 목적 함수에 기대되는 기여도
    *   <span>샘플링 될 확률 (1 또는 r)을 st라고 하고 (1/wt)</span>  
        ![](./ftrl_files/image2018-11-4_17-48-24.png "인텔리전스랩스 > 20181107_이상열_FTRL Proximal > image2018-11-4_17-48-24.png")
...
*   <span class="s1">모델 성능에 대한 포괄적인 이해를 가능하게하는 GridViz라고 하는 고차원적 대화형 시각화를 개발했습니다.</span>
    ...
    *   <span class="s1">셀의 색은 선**택한 기준선과 비교한 메트릭 값을 반영하므로** 전체 성능에 대한 시각적인 이해 뿐만 아니라 특이치 및 관심 영역에 대한 빠른 검색이 가능합니다.</span>
    ...
    <span class="s1">![](./ftrl_files/스크린샷 2018-11-06 오후 8.45.36.png "인텔리전스랩스 > 20181107_이상열_FTRL Proximal > 스크린샷 2018-11-06 오후 8.45.36.png")  
    </span>
...
*   **불확실성 점수라고 부르는 heuristic 방법**을 제안한다. 이것은 계산적으로 다루기 쉽고 경험적으로 예측 정확도를 정량화하는 일을 하는 부분
    ...
    *   ![](./ftrl_files/스크린샷 2018-11-05 오후 9.47.09.png "인텔리전스랩스 > 20181107_이상열_FTRL Proximal > 스크린샷 2018-11-05 오후 9.47.09.png")
...
*   <span class="s1">그림 3은 불확실성 스코어는 large 예측 에러와 높은 상관 관계가 있습니다. </span>
    ...
    <span class="s1">![](./ftrl_files/스크린샷 2018-11-05 오후 9.04.25.png "인텔리전스랩스 > 20181107_이상열_FTRL Proximal > 스크린샷 2018-11-05 오후 9.04.25.png")  
    </span>
...
*   **<span class="s1">체계적 편향 (일부 데이터 조각에 대한 예측 예측 및 관측 클릭률 (CTR)의 차이)가 발생하는 이유</span>**
    ...
    *   <span class="s1">부정확한 모델링 가정, 학습 알고리즘의 결함 또는 훈련 및 / 또는 검색에서 사용할 수 없는 숨겨진 기능과 같은 다양한 요소로 인해 발생할 수 있습니다 </span>
    ...
    <table class="wysiwyg-macro" data-macro-name="info" data-macro-id="8c374074-7f14-4a3b-82eb-66df73e2e896" data-macro-parameters="title=요약" data-macro-schema-version="1" style="background-image: url(/plugins/servlet/confluence/placeholder/macro-heading?definition=e2luZm86dGl0bGU97JqU7JW9fQ&amp;locale=ko_KR&amp;version=2); background-repeat: no-repeat;" data-macro-body-type="RICH_TEXT">
...
*   Aggressive Feature Hashing
    ...
    *   <span><span>이것은 우리에게 큰 비용 절감 효과를 주지 못했고, 대신 해석 가능한 (해쉬가없는) 특징 벡터를 유지하려고 했습니다.</span>  
        </span>
...
<pre>setwd('/home/syleeie/workspace/07_kaggle')
library(reticulate)
...
*   **캐글 데이터 문제(<span style="color: rgb(0,0,0);">Outbrain Click Prediction</span>) 적용하기**  
    ...
    *   FTRL 성능 비교 ([https://medium.com/unstructured/how-feature-engineering-can-help-you-do-well-in-a-kaggle-competition-part-iii-f67567aaf57c](https://medium.com/unstructured/how-feature-engineering-can-help-you-do-well-in-a-kaggle-competition-part-iii-f67567aaf57c))  
        ![](./ftrl_files/스크린샷 2018-10-16 오후 11.39.45.png "인텔리전스랩스 > 20181107_이상열_FTRL Proximal > 스크린샷 2018-10-16 오후 11.39.45.png")
    *   **FFM**
