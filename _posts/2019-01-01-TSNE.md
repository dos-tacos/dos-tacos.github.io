...
- Perplexity는 다음과 같이 정의된다. H(Pi)는 비트 단위로 측정 된 Pi의 Shannon 엔트로피
  - Perplexity가 높을 수록 분산이 높다고 해석 할 수 있음 
  - Perplexity는 2의 H승(p), i번째 점 주변의 가까운 점의 갯수, 일반적으로 5~50 상수로 정하고 모든 점에 대해서 Perplexity가 일정하게 유지되도록 하는 분산 값 할당 (binary search)
...
- 그림 7은 랜덤 워크 버전을 적용한 실험 결과
  ...
  - 모든 60,000 숫자를 사용하여 쌍의 유사성 pj|i 실험에서 k = 20 가까운 이웃들의 값을 사용하여 구성된 이웃 그래프를 사용했음
